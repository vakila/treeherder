...
def taskset(f):
    def inner(*args, **kwargs):
        task_set = kwargs.pop("_task_set", None)
        rv = f(*args, **kwargs)
        if task_set is not None:
            done = False
            with transaction.atomic():
                taskset_id = task_set["taskset_id"]
            ...
            if done:
                callback = Signature.from_dict(task_set["callback"])
...
def create_task_set(tasks, callback):
    tasks = tasks
    callback = callback
...
def parse_job_logs(project, tasks):
    ...
    for job_guid, task_list in tasks.iteritems():
        ...
        create_task_set(callback_group, callback)
